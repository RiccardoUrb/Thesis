...
class Drone:
    ...
    def check_goal(self):
        ...
        if self.potential_field_map[idx_x, idx_y] > 50 or self.goal_steps > self.max_steps_single_goal:
    ...
    def coverage_random(self):
        location_found = False
        repeat_counter = 0
        goal = np.array([np.random.random() * (self.map_dimension_x - 1), np.random.random() * (self.map_dimension_y - 1)])
        while not location_found:
            repeat_counter += 1
            goal = np.array([np.random.random() * (self.map_dimension_x - 1), np.random.random() * (self.map_dimension_y - 1)])
            idx_x = int(self.pos2index(goal[0]))
            idx_y = int(self.pos2index(goal[1]))
            if self.potential_field_map[idx_x, idx_y] < 50:
                location_found = True
            elif repeat_counter > 5:
                break
    ...
    def coverage_voronoi(self, drone_list):
        ...
        kmeans = KMeans(n_clusters=5, random_state=0).fit(unknown_points)
        ...
        # for goal_idx in range(0, len(exploration_area_centers)):
        ...
        #     drone_list[idx].update_attractive_layer()
        x_distances = np.ones(len(exploration_area_centers))*self.position[0] - np.array([exploration_area_centers[:, 0]])
        y_distances = np.ones(len(exploration_area_centers))*self.position[1] - np.array([exploration_area_centers[:, 1]])
        distances = np.hypot(x_distances, y_distances)[0]
        goal_idx = np.where(distances==np.min(distances))[0][0]
        #goal_idx = np.random.randint(0,len(distances))
        self.goal = exploration_area_centers[goal_idx, :]

    def coverage_RL(self):
        state = self.coverage_observe()
        state = self.adapt_coverage_state_dimension(state)
        state = tf.expand_dims(tf.convert_to_tensor(state), 0)
        new_goal = self.coverage_model(state).numpy()[0] * [self.map_dimension_x, self.map_dimension_y]
        new_goal = np.clip(new_goal, 0, self.map_dimension_x - 0.01)
    ...
    def adapt_coverage_state_dimension(self, state):
        ...
        elif state_dim_x%self.coverage_observation_size == 0 and state_dim_x%self.coverage_observation_size == 0:
    ...
    def interpolate_trajectory(self):
        x = 0
        y = 1
        polynomial_degree = 3                           # polynomial degree of the interpolation
    ...
    def compute_vertex(self, close_obstacles_list):
        hull = ConvexHull(close_obstacles_list)
        vertex = np.unique(np.reshape(np.array(close_obstacles_list[hull.simplices]), [-1,2]), axis=0)
        self.fill_rectangles(vertex)
    ...
    def action(self, drone_list):
        ...
        if not self.goal_updated:
            if np.count_nonzero(self.covered_area_matrix) / self.total_explorable_elements <= 0.3:
                self.coverage_random()
            elif np.count_nonzero(self.covered_area_matrix) / self.total_explorable_elements <= 0.9:
                self.coverage_RL()
            else:
                self.coverage_voronoi(drone_list)
            self.update_attractive_layer()
