...
class Drone:
    def __init__(self):
        ...
        self.starting_position = np.empty([0, 3], dtype='float16')
    ...
    def set_initial_position(self, intial_position):
        ...
        self.starting_position = intial_position
    ...
    def coverage_random(self):
        ...
        goal = np.array([np.random.random() * (self.map_dimension_x - 1), np.random.random() * (self.map_dimension_y - 1), 2])
        while not location_found:
            repeat_counter += 1
            goal = np.array([np.random.random() * (self.map_dimension_x - 1), np.random.random() * (self.map_dimension_y - 1), 2])
    ...
    def detect_obstacles(self):
        ...
        def callback(ros_point_cloud, ):
            ...
            for data in int_data:
                obstacle_position_x = np.round(data[0] + self.starting_position[x], 2)
                obstacle_position_y = np.round(data[2] + self.starting_position[y], 2)
                obstacle_position_z = np.round(-data[1] + self.starting_position[z], 2)
                self.newfound_obstacle_list = np.vstack([self.newfound_obstacle_list, [obstacle_position_x, obstacle_position_y, obstacle_position_z]])
        ...
        for theta_angle in np.linspace(-180, 180, self.proximity_sensor_resolution):
            ...
            for phi_angle in np.linspace(-180, 180, self.proximity_sensor_resolution):
                ...
                while distance <= proximity_sensor_cells_range:
                    ...
                    idx_y = int(math.floor(self.index_position[y] + distance * math.sin(phi) * math.sin(theta)))
                    idx_z = int(math.floor(self.index_position[z] + distance * math.cos(phi)))
                    if self.obstacle_matrix[idx_z, idx_x, idx_y] == 1:
